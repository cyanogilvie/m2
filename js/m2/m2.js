	...
	var api = {
		...
		_got_msg: function(msg) { //<<<
			...
			switch (msg.type) {
				...
				case 'pr_jm':
					jm.setItem(msg.prev_seq, jm.getItem(msg.prev_seq) + 1);
				case 'ack':
				case 'nack':
				case 'jm':
					this._response(msg);
		...
		switch (type) {
			...
			case 'disconnected':
				...
				try {
					...
					console.log('dispatching disconnected event went badly: '+e);
				}
				break;
			case 'waiting':
				break;
			case 'failed':
				break;
			default:
				console.log('Unhandled socket status: "'+type+'"');
				break;
		}
	}

	socket.onLoaded = function(data) {
